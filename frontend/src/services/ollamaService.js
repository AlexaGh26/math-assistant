 ...
 * Se conecta al backend que maneja la comunicación con modelos de lenguaje
 */
...
export const generateResponse = async (prompt, model = DEFAULT_MODEL) => {
  try {
    ...
    console.warn('Error al conectar con el backend:', backendError);
    
    // Si el backend falla, usar el mecanismo de respuesta fallback
...
const getFallbackResponse = (question) => {
  ...
  // Buscar palabras clave para determinar el tema
  ...
  // Para operaciones matemáticas simples
  ...
  if (mathExpression) {
    ...
    if (result !== undefined) {
      // Redondear a 2 decimales si es un decimal
...
const extractMathExpression = (text) => {
  // Buscar patrones como "2 + 3", "cuánto es 5 por 6", etc.
  const patterns = [
    /(\d+)\s*\+\s*(\d+)/,          // 2 + 3
    /(\d+)\s*\-\s*(\d+)/,          // 8 - 5
    /(\d+)\s*(?:\*|x|×)\s*(\d+)/,  // 4 * 6 o 4 x 6
    /(\d+)\s*(?:\/|÷)\s*(\d+)/     // 10 / 2 o 10 ÷ 2
  ];
  ...
  // Buscar expresiones en lenguaje natural
